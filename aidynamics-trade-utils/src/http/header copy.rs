...
use crate::{
    ...
    sys::{local_ip, public_ip},
    Result,
};

/// Http headers to be sent to the API
#[derive(Debug,Clone)]
pub struct HttpHeader(pub HeaderMap);
...
impl HttpHeader {
        ...
        J: Display,
    {
        ...
        // ---- TEMPORARY COMMENTED, BECAUSE IT IS NOT PROVIDED BY SERVER ----
        // headers.insert("X-MACAddress", mac_addr.parse().unwrap());

        headers.insert("X-MACAddress", "fe80::216e:6507:4b90:3719".parse().unwrap());
