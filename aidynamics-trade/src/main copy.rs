...
use aidynamics_trade::{
    ...
    trade_handler::{Segment, Strategy, TradeHandler, TradeHandlerConfig},
...
use aidynamics_trade::trade_handler::{self, Payload, Trade, TradeStatus};
use tokio::sync::mpsc;
...
enum Signal {
    ...
    #[serde(rename = "new_trade_handler")]
    NewTradeHandler,

    /// This will be forwarded to channel to add to handler vec
    AddNewTradeHandler(TradeHandlerConfig),
    ...
    NewTrade{new_trade_res:NewTradeRes},
}
...
async fn add_trade_handler(config: TradeHandlerConfig) -> Result<TradeHandler> {
    println!("Adding handlers...");
    let new_handler = TradeHandler::new_from_session(config).await.unwrap();
...
async fn login_web_server(
    ...
    let url = "https://api.aidynamicsplatform.in/serverlogin"; // Example URL, replace with your actual endpoint

    println!("rust pwd = {:?}", rust_password);
...
impl WebSocketClient {
    ...
    /// Here tx parameter is to send message to channel in main thread
    async fn connect(
        ws_url: String,
        ...
        tokio::task::spawn(async move {
            ...
            if let Err(e) = write.send(ws_message::Text(serialized_message)).await {
            ...
            let read_task = tokio::spawn(async move {
                while let Some(msg) = read.next().await {
                    match msg {
                        Ok(message) => {
                            // Process the message and attempt to deserialize it into `Signal`
                            match message {
                                ws_message::Text(text) => {
                                    match serde_json::from_str::<NewMessage>(&text) {
                                        Ok(msg) => {
                                            ...
                                            match msg.signal {
                                                Signal::Auth => {
                                                    // TradeHandlerConfig will be signal here
                                                    println!("Auth data : {:?}", msg.data);
                                                    // let auth =
                                                    //     serde_json::from_str::<AuthResponse>(
                                                    //         msg.data,
                                                    //     )
                                                    //     .unwrap();
                                                    // println!("{:?} {:?}",auth.auth_status, auth.message);
                                                }
                                                Signal::NewTradeHandler => {
                                                    // TradeHandlerConfig will be signal here
                                                    println!(
                                                    ...
                                                    // add_trade_handler(config)
                                                    // let new_th_config = serde_json::from_str::<TradeHandlerConfig>(msg.data).unwrap();
                                                }
            ...
            // Listen for messages from the main thread and send them to the WebSocket
            // tokio::spawn(async move {
            while let Some(message) = rx.recv().await {
                // if let Err(e) = write.send(message).await {
                if let Err(e) = write.send(ws_message::Text("".to_string())).await {
            ...
            // Spawn a task to send ping messages periodically
            // let mut write_clone = write.clone();
            tokio::spawn(async move {
                loop {
                    sleep(Duration::from_secs(30)).await;
                    if let Err(e) = write.send(ws_message::Ping(vec![])).await {
                        println!("Error sending ping: {:?}", e);
                        println!("BROKEN ON RECEIVING PING");
                        break;
                    }
                    println!("Sent ping");
                }
...
async fn main() {
    ...
    println!("RUST PASSWORD IS ============ {:?}", &rust_password);
    ...
    async fn receive_messages(rx_main: &mut Receiver<Signal>, tx_main_clone: Sender<Signal>) {
        let mut trade_handlers: Vec<TradeHandler> = Vec::new();
        while let Some(msg) = rx_main.recv().await {
            ...
            match msg {
                    ...
                    for handler in trade_handlers.iter_mut() {
                        if is_index {
                                ...
                                && handler.trade_id == 0
                            {
                                ...
                                {
                                    handler.wait = true;
                                    // HERE IT WILL BLOCK THE THREAD
                                    handler.place_trade().await;
                                    handler.wait = false;
                ...
                Signal::OrderPlaced{order_data} => {
                    println!("Error add trade handler... in main channel {:?}", e);
                }
                Signal::AddNewTradeHandler(config) => match add_trade_handler(config).await {
                    Ok(trade_handler) => {
                ...
                _ => {
                    println!("Other signal received inside....");
                }
            }
        }
    }
    ...
    tokio::spawn(async move {
        receive_messages(&mut rx_main, tx_main_cln).await;
        sleep(Duration::from_secs(5));
    });

    tx_main.send(Signal::AddNewTradeHandler(TradeHandlerConfig {
        trade_handler_id: 1,
        api_key: api_key_clone,
        client_code: client_code_clone,
        pin: pin_clone,
        jwt_token: jwt_token_clone,
        refresh_token: refresh_token_clone,
        feed_token: feed_token_clone,
        max_loss,
        max_price,
        max_trades,
        symbol,
...
async fn add_trade_handler(
    // shared_tradehandlers: &mut Vec<TradeHandler>,
    ...
    println!("Adding handlers...");
    let new_handler = TradeHandler::new_from_session(TradeHandlerConfig {
...
fn create_message_handler(
    instances: Arc<Mutex<Vec<TradeHandler>>>,
...
async fn handle_ws_message(message: &NewMessage, instances: &mut Arc<Mutex<Vec<TradeHandler>>>) {
    match &message.signal {
        Signal::NewTradeHandler => {
            match &message.data {
                Value::Array(trades) => {
                    let data_received: Vec<TradeHandlerConfig> = trades
                        .iter()
...
// async fn handle_place_order(handler: &mut TradeHandler) {
...
async fn place_order(
    handler: &mut TradeHandler,
) -> std::result::Result<TradeHandler, Box<dyn std::error::Error>> {
    if handler.trade_status == TradeStatus::Closed {
        handler.wait = true;
        let entry_req = PlaceOrderReq::new(
            "KOKUYOCMLN-EQ",
...
// async fn handle_price_feed(shared_instance:&mut Arc<Mutex<Vec<TradeHandler>>>,msg:Message){
...
async fn sync_tradehandler(rx: &mut Receiver<TradeHandler>,trade_handler_ref:&mut Arc<Mutex<Vec<TradeHandler>>>) {
...
#[tokio::main]
async fn main() {
    let (tx_tradehandler,mut rx_tradehandler) = tokio::sync::mpsc::channel::<TradeHandler>(100);
    ...
    let mut shared_tradehandlers = Arc::new(Mutex::new(Vec::<TradeHandler>::new()));
    let mut auth_token = "".to_string();
    // let mut web_server_token = "";
    let mut angelone_tokens: Option<Value> = None;
    ...
    let mut handlers = {
        let handlers = shared_tradehandlers.lock().await;
        // handlers.clone() // Clone the entire vector so we can release the lock
        let hndl: Vec<TradeHandler> = handlers
            .clone()
            .into_iter()
            .filter(|h| !h.stopped)
            .collect();
