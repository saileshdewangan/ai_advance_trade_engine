...
use super::websocket_client::Signal;
...
impl AngelOneWebSocketClient {
    ...
    pub async fn connect(
        ...
        tx: mpsc::Sender<super::websocket_client::Signal>,
        // on_message:impl Fn(Ltp)
    ) {
                        ...
                        match msg {
                            Ok(message) => match message {
                                Message::Binary(bin_msg) => {
                                    ...
                                    // println!("PARSED  = {:?}", ltp_data);
                                    // println!("Ltp = {:?}", ltp_data.ltp);
                                    // on_message(ltp_data);
                                    let is_index = if ltp_data.exchange_type == 1 {
                                                true
                                            } else {
                                                false
                                            };
