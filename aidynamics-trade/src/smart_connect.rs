...
use aidynamics_trade_utils::http::{HttpClient, HttpFetcher, HttpSender, INSTRUMENT_URL};
...
impl SmartConnect {
    ...
    /// Returns a new instance for the smart connect API with session
    pub async fn new_with_session<A, C, P, J, R, F>(
        api_key: A,
        client_code: C,
        pin: P,
        jwt_token: J,
        ...
        J: Display,
    {
        ...
        let http = HttpClient::new_with_jwt(&api_key, jwt_token).await?;
        Ok(Self {
            api_key,
            client_code: "".into(),
            pin: "".into(),
    ...
    /// Set new jwt_token and api
    pub fn set_auth_and_key(&mut self, jwt_token: &str, api_key: &str) -> Result<()> {
        self.http.set_auth_and_key(jwt_token, api_key).unwrap();
        Ok(())
    }
    ...
    /// Generates the session to receive authentication tokens and user information USING TOTP directly
    pub async fn generate_session_with_totp<O>(&mut self, totp: O) -> Result<()>
    where
        O: Into<String>,
    {
        let session_req = SessionReq::new_with_totp(&self.client_code, &self.pin, totp).await?;
    ...
    pub async fn place_order(&self, order_req: &PlaceOrderReq) -> Result<PlaceOrderRes> {
        let response = order_req.send_data(&self.http).await;
        response
    }
